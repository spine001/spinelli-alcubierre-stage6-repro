...
SESSION="${SPINELLI_RESUME_TMUX:-spinelli-stage4-resume}"
LINK="$REPO/results/stage4_resume_N141_to_N201_latest"
...
snapshot() {
  ...
  STATE="NOT_STARTED"
  NOTE="No continuation batch detected."

  if [[ -n "$PID" ]]; then
    STATE="RUNNING_N${CURRENT_N}"
    NOTE="The actual Python process is active."
  elif [[ -n "$ROOT" &&
          -f "$ROOT/stage4_resume_N141_to_N201_manifest.json" ]]; then
    STATE="$(
      python3 - "$ROOT/stage4_resume_N141_to_N201_manifest.json" <<'PY'
import json
import sys
print(json.load(open(sys.argv[1]))["batch_status"])
PY
    )"
    NOTE="The batch is between cases, completed, or stopped with a recoverable package."
  fi

  clear 2>/dev/null || true
  line
  echo " SPINELLI STAGE 4 SWAP-AUTHORIZED RESUME N141 TO N201"
  line
  printf ' Time:                    %s\n' "$(date -Is)"
  printf ' State:                   %s\n' "$STATE"
  ...
  printf ' Process-swap ceiling:    %s GiB\n' "$MAX_SWAP_GIB"
  printf ' Tmux:                    %s\n' \
  ...
  awk -v limit="$MAX_SWAP_GIB" '
    ...
    END {
      ...
      printf " Swap used:               %.2f GiB of %.2f GiB\n",
        used, total/1048576
      printf " Swap authorization used: %.2f%%\n",
        100*used/limit
    }
  ' /proc/meminfo
  ...
  if [[ -n "$PID" && -r "/proc/$PID/status" ]]; then
    ...
    awk -v limit="$MAX_SWAP_GIB" '
      ...
      END {
        ...
        printf " Process swap ceiling:   %.2f%%\n",
          100*(swap/1048576)/limit
        printf " Threads:                %s\n", threads
      }
    ' "/proc/$PID/status"
  fi
  ...
  if [[ -f "$CURRENT_SAMPLES" ]]; then
    awk -F, -v limit="$MAX_SWAP_GIB" '
      ...
      END {
        ...
        printf " Max process swap limit:  %.2f%%\n",
          100*(swap/1048576)/limit
  ...
  echo " CASE STATUS"
  subline
  echo " N121: reused PASS"
  echo " N131: reused PASS"
  if [[ -n "$ROOT" ]]; then
    for N in 141 151 161 171 181 191 201; do
      REPORT="$ROOT/N$N/stage4_n${N}_swap_enabled_report.txt"
      if [[ -f "$REPORT" ]] &&
         grep -q "^STAGE4_N${N}_SWAP_ENABLED_RUN_RESULT=PASS$" \
           "$REPORT"; then
        echo " N${N}: PASS"
      elif [[ -d "$ROOT/N$N" ]]; then
        echo " N${N}: attempted/in progress"
      else
        echo " N${N}: pending"
      fi
    done
  ...
  subline
  [[ -n "$ROOT" &&
     -f "$ROOT/stage4_resume_N141_to_N201.run.log" ]] &&
    tail -n40 "$ROOT/stage4_resume_N141_to_N201.run.log" ||
    echo " No continuation log."
  line
}
