...

{
  ...
  [[ -x "$PYTHON" ]] || { echo "ERROR: venv Python missing: $PYTHON"; exit 1; }
  ...
  "$PYTHON" - <<'PY'
import sys, numpy, pandas, matplotlib, scipy, psutil
...
PY
  ...
  grep -q '^STAGE4_N61_REGRESSION_RESULT=PASS$' \
    "$N61/stage4_n61_exact_regression_report.txt" || {
      echo "ERROR: N61 PASS report missing"; exit 1;
    }
  ...
  awk -v used="$SWAP_USED_KIB" -v total="$SWAP_TOTAL_KIB" \
  ...
  [[ "$FREE_GIB" -ge 100 ]] || {
    echo "ERROR: less than 100 GiB filesystem space is available."; exit 1;
  }
  ...
  MATCHES="$(
    pgrep -af \
      'python.*(run_stage4_n61_exact_regression|run_stage4_n71_three_grid_analysis|run_stage4_n81_dense_convergence|stage6_alcubierre|run_stage6E)' \
      || true
  )"
  if [[ -n "$MATCHES" ]]; then
    echo "$MATCHES"
    echo "ERROR: matching Python computation appears active."
    exit 1
  fi
