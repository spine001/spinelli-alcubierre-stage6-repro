...
ROBUST="$REPO/scripts/stage6_reaggregate_robust_env.py"
...
N_LIST="${STAGE6E_N_LIST:-261}"
VS_LIST="${STAGE6E_VS_LIST:-0.5}"
...
cat > "$OUTDIR/run_config.json" <<CONFIG
{
  ...
  "branch_continuation_acceptance": {
    "N261_v0p5": {
      ...
      "rho_relative_peak_error": [0.0, 0.06],
...
echo "============================================================" | tee -a "$QUEUE_LOG"
echo "Stage 6E robust branch-continuation run started at $(date)" | tee -a "$QUEUE_LOG"
