#!/usr/bin/env python3
"""Memory-profiled streaming Stage 4 regression runner.
...
def main() -> int:
    ...
    parser.add_argument("--target-n", required=True, type=int)
    ...
    allowed = {
        f"stage4_streaming_N{args.target_n}.run.log",
    }
    unexpected = [
    ...
    try:
        ...
        comparison_pass, comparisons = compare_tables(
            output,
            reference,
            args.rtol,
            args.atol,
        )
        metadata["table_comparisons"] = comparisons
        ...
        report_json = (
            output
            / f"stage4_streaming_N{args.target_n}_report.json"
        )
        report_txt = (
            output
            / f"stage4_streaming_N{args.target_n}_report.txt"
        )
        profile_csv = (
            output
            / f"stage4_streaming_N{args.target_n}_memory_profile.csv"
        )
        comparison_csv = (
            output
            / f"stage4_streaming_N{args.target_n}_comparison.csv"
        )
        ...
        lines = [
            (
                f"===== STAGE 4 STREAMING EXPORT REGRESSION "
                f"N={args.target_n} ====="
            ),
        ...
        for item in comparisons:
            lines.append(
                f"{item['table']}: status={item['status']} "
                f"max_abs={item['max_abs_difference']:.17g} "
                f"max_rel={item['max_relative_difference']:.17g} "
                f"outside={item['numeric_outside_tolerance']} "
                f"text={item['text_mismatches']}"
            )
        lines += [
            "",
            (
                "STAGE4_STREAMING_EXPORT_REGRESSION_RESULT="
                f"{metadata['run_status']}"
            ),
        ]
