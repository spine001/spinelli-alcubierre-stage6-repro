#!/usr/bin/env python3
"""Optimized Stage 4 N111 swap-enabled production runner.
...
def main() -> int:
    ...
    parser.add_argument("--target-n", type=int, default=111)
    parser.add_argument("--memory-budget-gib", type=float, default=512.0)
    parser.add_argument("--delta-tau", type=float, default=0.04)
    parser.add_argument("--rtol", type=float, default=1e-8)
    parser.add_argument("--atol", type=float, default=1e-10)
    args = parser.parse_args()

    if args.target_n != 111:
        raise SystemExit(
            f"This production runner is locked to N=111; got {args.target_n}"
        )
    ...
    try:
        ...
        report_json = output / "stage4_n111_optimized_swap_enabled_report.json"
        report_txt = output / "stage4_n111_optimized_swap_enabled_report.txt"
        profile_csv = output / "stage4_n111_optimized_swap_enabled_memory_profile.csv"
        metrics_csv = output / "stage4_n111_optimized_swap_enabled_metrics.csv"
        ...
        lines = [
            "===== STAGE 4 N111 OPTIMIZED SWAP-ENABLED PRODUCTION RUN =====",
        ...
        lines += [
            "",
            "STAGE4_N111_SWAP_ENABLED_RUN_RESULT=PASS",
        ]
