#!/usr/bin/env python3
"""Swap-authorized Stage 4 continuation case runner.
...
This runner accepts authorized continuation grids N141 through N201 and emits canonical tables, metrics, and a memory profile.
"""
...
def main() -> int:
    ...
    parser.add_argument("--target-n", type=int, required=True, choices=[141, 151, 161, 171, 181, 191, 201])
    parser.add_argument("--memory-budget-gib", type=float, default=4096.0)
    parser.add_argument("--delta-tau", type=float, default=0.04)
    parser.add_argument("--rtol", type=float, default=1e-8)
    parser.add_argument("--atol", type=float, default=1e-10)
    args = parser.parse_args()

    allowed_targets = {141, 151, 161, 171, 181, 191, 201}
    if args.target_n not in allowed_targets:
        raise SystemExit(
            "This batch runner accepts only "
            f"{sorted(allowed_targets)}; got {args.target_n}"
        )

    prefix = f"stage4_n{args.target_n}_swap_enabled"
    run_log_name = f"{prefix}.run.log"
    samples_name = (
        f"stage4_n{args.target_n}_resource_samples.csv"
    )
    ...
    target_max_n_cap = f"MAX_N_CAP = {args.target_n}"
    ...
    patched_cell3 = (
        ...
        .replace(OLD_MAX_N_CAP, target_max_n_cap, 1)
    ...
    metadata: dict[str, Any] = {
        ...
        "swap_authorization": {
            "max_process_swap_gib": 2990.0,
            "max_authorized_n": 201,
            "selector_max_n_cap": args.target_n,
            "selector_budget_gib": args.memory_budget_gib,
        },
        "cells": [],
    }
    ...
    try:
        ...
        report_json = output / f"{prefix}_report.json"
        report_txt = output / f"{prefix}_report.txt"
        profile_csv = output / f"{prefix}_memory_profile.csv"
        metrics_csv = output / f"{prefix}_metrics.csv"
        ...
        lines = [
            f"===== STAGE 4 N{args.target_n} OPTIMIZED SWAP-ENABLED PRODUCTION RUN =====",
        ...
        lines += [
            "",
            f"STAGE4_N{args.target_n}_SWAP_ENABLED_RUN_RESULT=PASS",
        ]
